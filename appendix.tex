...
\section{手段}
進行方向の決定に必要な下記の3つの計算回数を削減する．

\begin{itemize}
\item エージェント間距離の計算
\item 進行方向ベクトル$e$の計算
\item 障害物を避ける力$F_{iw}$の計算
\end{itemize}
...
\begin{itemize}
\item \textbf{進行方向の計算回数削減によるSFMの高速化に関する研究}
\item 進行方向計算中の演算回数削減によるSFMの高速化に関する研究
\item 進行方向計算のための演算回数削減によるSFMの高速化に関する研究
\end{itemize}
